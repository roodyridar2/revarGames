...
const ThreeMensMorris = () => {
  ...
  const translations = {
    english: {
      ...
      blackPlayer: "Rock",
      whitePlayer: "Paper",
      blackPieces: "Rock Pieces",
      whitePieces: "Paper Pieces",
      wins: "Wins!",
      newGame: "New Game",
      ...
      movementTip:
        "Tip: Click on your piece to select it, then click on a valid position to move.",
    },
    kurdish: {
      title: "یاری بەرد و کاغەز  ",
      turn: "نۆرەی {player}",
      placementPhase: "قۆناغی دانان",
      movementPhase: "قۆناغی جووڵان",
      blackPlayer: "بەرد",
      whitePlayer: "کاغەز",
      blackPieces: "دانەکانی بەرد",
      whitePieces: "دانەکانی کاغەز",
      wins: "بردیەوە!",
      newGame: "یاری نوێ",
      ...
      movementTip:
        "ئامۆژگاری: کلیک لەسەر دانەکەت بکە بۆ هەڵبژاردنی، پاشان کلیک لەسەر شوێنێکی گونجاو بکە بۆ جووڵاندنی.",
    },
  };
  ...
  // Render the game pieces using rock and paper images
  const renderPieces = () => {
    return board.map((piece, position) => {
      ...
      const imageSize = 40; // Size of the images
      
      return (
        <motion.image
          key={`piece-${position}`}
          href={piece === "black" ? rock : paper}
          x={pos.x - imageSize/2}
          y={pos.y - imageSize/2}
          width={imageSize}
          height={imageSize}
    ...
    return positions.map((pos, index) => (
      <motion.circle
        ...
        strokeDasharray={
          board[index] === null &&
          selectedPiece !== null &&
          isValidMove(selectedPiece, index)
            ? "3,3"
            : "0"
        }
        whileHover={
          board[index] === null
            ? { scale: 1.2, fill: "rgba(203, 213, 225, 0.2)" }
            : {}
        }
  ...
  // Render unplaced pieces using rock and paper images
  const renderUnplacedPieces = () => {
    const unplacedPieces = [];
    const imageSize = 36; // Size of the unplaced pieces

    // Black pieces (rocks) to place
    for (let i = 0; i < blackPieces; i++) {
      unplacedPieces.push(
        <motion.image
          key={`unplaced-black-${i}`}
          href={rock}
          x={350 + i * 40 - imageSize/2}
          y={100 - imageSize/2}
          width={imageSize}
          height={imageSize}
  ...
  return (
    <div
      className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-indigo-50 to-blue-100"
      dir={language === "kurdish" ? "rtl" : "ltr"}
    >
      ...
      <div className="mb-6 bg-white p-4 rounded-xl shadow-md w-[550px] text-center">
        {!winner && (
          <div className="flex items-center justify-center gap-4">
            <div className="w-8 h-8 flex items-center justify-center">
              <img 
                src={currentPlayer === "black" ? rock : paper} 
                alt={currentPlayer === "black" ? "Rock" : "Paper"} 
            ...
            >
              <span className="text-gray-700">
                {t.turn.replace(
                  "{player}",
                  currentPlayer === "black" ? t.blackPlayer : t.whitePlayer
                )}
              </span>
            </div>
          </div>
        ...
        <div
          className={`mt-2 text-lg font-medium text-indigo-600 ${
            language === "kurdish" ? "font-kurdish" : ""
          }`}
        >
      ...
      <div className="relative bg-white rounded-2xl shadow-xl p-6 mb-8">
        <svg width="500" height="300" viewBox="0 0 500 300">
          ...
          {/* Labels */}
          <text x="335" y="80" className="text-sm font-medium" fill="#475569">
            {t.blackPieces}
          </text>
          <text x="335" y="180" className="text-sm font-medium" fill="#475569">
            {t.whitePieces}
          </text>
        </svg>
      </div>
      ...
      <div className="space-y-6 max-w-md">
        <button
          onClick={resetGame}
          className={`px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition duration-300 shadow-md font-medium ${
            language === "kurdish" ? "font-kurdish" : ""
          }`}
        >
          {t.newGame}
        ...
        <div
          className={`bg-white p-4 rounded-xl shadow-md text-gray-700 ${
            language === "kurdish" ? "font-kurdish" : ""
          }`}
        >
      ...
      {winner && (
        <div className="fixed inset-0 bg-black/50 bg-opacity-50 flex items-center justify-center z-10">
          <motion.div
            ...
            className={`bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full m-4 text-center ${
              language === "kurdish" ? "font-kurdish" : ""
            }`}
          >
            ...
            <div className="mb-6">
              <img 
                src={winner === "black" ? rock : paper} 
                alt={winner === "black" ? "Rock" : "Paper"} 
